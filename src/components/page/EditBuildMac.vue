...
<script>
	...
	export default {
		...
		methods: {
			onSubmit() {
				net.post(api.modifyBuildMac(), this.form, result => {
					const { name, ip } = this.form;
					if (name === '') {
						this.$message.error('打包机的名称不能为空！');
						return;
					}
					if (ip === '') {
						this.$message.error('打包机的IP不能为空！');
						return;
					}
					if (result.status === 1) {
		...
		mounted: function () {
			net.get(api.getBuildMacByID(this.$route.query.id), result => {
				if (result.status === 1) {
